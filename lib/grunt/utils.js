...
var errorCompressor = require('../error-compressor/compressor');
...
module.exports = {
  ...
  startJasmineNode: function(folder, done){
    var p = spawn('node_modules/jasmine-node/bin/jasmine-node', [folder]);
  ...
  min: function(file, done) {
    var errorCompress = false;

    if (file.src) {
      errorCompress = file.errorCompress || false;
      file = file.src;
    }
    ...
    if (errorCompress) {
      var slimFile = file.replace(/\.js$/, '.slim.js');
      var errorFile = file.replace(/\.js$/, '.err');

      errorCompressor.compress(file, slimFile, errorFile);
    }
