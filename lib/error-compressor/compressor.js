...
function main(argv) {
  ...
  if (cmd_line_arguments.length == 3) {
    compress(cmd_line_arguments[0], cmd_line_arguments[1], cmd_line_arguments[2])
  } else {
...
function compress(sourceFile, destinationFile, errorFile) {
  var compressor = new Compressor();
  var file_contents = compressor.readAndGetFileContents(sourceFile);
  var ast = compressor.parseJsFileAndGetAst(file_contents);
  compressor.extractErrorsTable(ast);
  compressor.overwriteFile(destinationFile, ast);
  compressor.writeErrorFile(errorFile);
}
...
Compressor.prototype.overwriteFile = function(file_name, ast) {
  fs.writeFileSync(file_name, pro.gen_code(ast, {beautify: true, indent_start: 0, indent_level: 2}));
};
